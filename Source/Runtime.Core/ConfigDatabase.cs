	...
	public static class ConfigDatabase
	{
		...
		public static string GetConfigFileName()
		{
			...
			if (string.IsNullOrWhiteSpace(configFileName))
			{
				...
				configFileName = ".beamable/config-defaults.json";
#else
...
#if GODOT4_0_OR_GREATER
			return Godot.FileAccess.FileExists($"res://{filename}");
#elif UNITY_EDITOR
		...
		public static string GetString(string name)
		{
			...
			{
				if (sessionOverrides.TryGetValue(name, out var s))
				{
					return s;
				}
		...
		private static string GetFileContent(string fileName)
		{
			...
			{
				var file = Godot.FileAccess.Open($"res://{fileName}", Godot.FileAccess.ModeFlags.Read);
